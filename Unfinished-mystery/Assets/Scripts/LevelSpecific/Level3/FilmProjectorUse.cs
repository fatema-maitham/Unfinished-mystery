   ...
   private void Start()
{
    ...
    private int GetBestReelAction()
    {
        if (!reel1Watched)
        {
            if (InventoryHasItem(reel1Item)) return 1;
            if (InventoryHasItem(reel2Item)) return 2;
            if (InventoryHasItem(reel3Item)) return 3;
            return 1;
        }
   ...
   private void OnVideoFinished(VideoPlayer vp)
    {
        ...
        else if (currentReelNumber == 2 && !reel2Watched)
        {
            ...
            StartCoroutine(ShowDiscoveryThenPrompt(
                "Scene 2 discovered:\nSomeone blocked the exit that night.\nFind the final reel."
            ));
        }
        else if (currentReelNumber == 3 && !reel3Watched)
        {
            ...
            StartCoroutine(ShowDiscoveryThenPrompt(
                "Final reel discovered:\nMaya knew the truth and chose silence.\nThe exit is unlocked."
            ));
        }
        else
        {
